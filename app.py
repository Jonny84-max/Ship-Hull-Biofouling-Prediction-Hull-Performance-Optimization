...
st.write("""
A machine learning system that predicts hull biofouling severity (Low, Medium, High)
and recommends maintenance actions to optimize hull performance and reduce fuel consumption.
""")
...
plt.figure()
plt.plot(fouling_range, fuel_values)
plt.xlabel("Hull Roughness (mm)")
plt.ylabel("Fuel Consumption (kg/hr)")
plt.title("Fuel Consumption vs Hull Fouling")
st.pyplot(plt)
...
plt.figure()
plt.plot(fouling_range, speed_values)
plt.xlabel("Hull Roughness (mm)")
plt.ylabel("Speed (kn)")
plt.title("Speed Loss vs Hull Fouling")
st.pyplot(plt)
